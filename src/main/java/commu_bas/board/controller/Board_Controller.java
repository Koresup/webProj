...
public class Board_Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serviceStr = request.getRequestURI().substring(
				(request.getContextPath() + "/commu_bas/board/").length()
				);
		...
		if (nonClass.contains(serviceStr)) {
			request.setAttribute("mainUrl", "commu_bas/board/"+serviceStr);

		} else {
			// 외워라
			try {
				BoardService service = 
						(BoardService) Class.forName("commu_bas.board.service.Board" + serviceStr)
